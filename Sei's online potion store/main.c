...
void sum_total_costs(order_t orders[], unsigned order_cnt){
    for(int i = 0; i < order_cnt; i++){
        ...
        for (int j = 0; j < orders[i].item_cnt; j++){
            cost += orders[i].items[j].cost * orders[i].items[j].cnt;
        }
        orders[i].total_cost = cost;
    }
}
void reorder(order_t orders[], unsigned order_cnt){
    order_t tmp_order;
    ...
    for(int i = 0; i < order_cnt; i++){
        ...
        for (int j = i; j < order_cnt; j++){
            if (tmp_order.pick_up_time.hour > orders[j].pick_up_time.hour){
                tmp_order = orders[j];
                k = j;
            }
            else if (tmp_order.pick_up_time.hour ==  orders[j].pick_up_time.hour && tmp_order.pick_up_time.minute >= orders[j].pick_up_time.minute){
                tmp_order = orders[j];
                k = j;
            }
        }
